...
abstract class H2OContextImplicits {
  ...
  implicit def asH2OFrameFromRDDMmlVector(rdd: RDD[ml.linalg.Vector]): H2OFrame = _h2oContext.asH2OFrame(rdd, None)
  ...
  implicit def toH2OFrameKeyfromRDDMmlVector(rdd: RDD[ml.linalg.Vector]): Key[_] = _h2oContext.toH2OFrameKey(rdd, None)
